...
Release versioning rule for this repo: do not hand-edit `package.json.version` in normal feature/fix PRs. Version bumps are release actions only, must be created with `npm version patch|minor|major`, and should be published by the tag-triggered CI workflows.
